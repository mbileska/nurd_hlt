...
def scan_legacy_same_sample(loss_1, loss_2, percent, min_A=50, min_D=500,
                            weights=None):
    """Reproduce the historical optimize-and-report-on-one-sample scan.

    This is deliberately an oracle diagnostic, not a model-selection metric.
    Unlike historical runs, its quantiles and yields use generator weights.
    """
    return scan_abcd_grid(
        loss_1, loss_2, percent,
        min_A=min_A,
        min_D=min_D,
        min_A_frac=0.0,
        ...
        min_region_frac=0.0,
        max_ratio_unc=np.finfo(np.float64).max,
        selection_folds=1,
        weights=weights,
    )
...
def class_assignment_diagnostics(true_labels, score_products, weights=None):
    ...
    weights = (
        np.ones(len(true_labels), dtype=np.float64) if weights is None
        else np.asarray(weights, dtype=np.float64).reshape(-1)
    )
    if weights.shape != true_labels.shape:
        raise ValueError("Class-assignment weights must align with labels.")
    result = {}
    for name, key in (
        ...
        weighted_matrix = np.zeros(
            (len(reference_labels), len(reference_labels)), dtype=np.float64)
        for row, true_label in enumerate(reference_labels):
            for column, predicted_label in enumerate(reference_labels):
                ...
                matrix[row, column] = np.count_nonzero(cell)
                weighted_matrix[row, column] = weights[cell].sum()
        valid_weight = weights[valid]
        row_weight = weighted_matrix.sum(axis=1)
        per_class_accuracy = np.divide(
            np.diag(weighted_matrix), row_weight,
            out=np.full(len(reference_labels), np.nan), where=row_weight > 0)
        result[name] = {
            "measure": "generator_weighted",
            "accuracy": (
                float(np.sum(
                    valid_weight * (predicted[valid] == true_labels[valid]))
                    / valid_weight.sum())
                if valid.any() and valid_weight.sum() > 0 else np.nan
            ),
            "balanced_accuracy": float(np.nanmean(per_class_accuracy)),
            "per_class_accuracy": per_class_accuracy.tolist(),
            "confusion_matrix": matrix.tolist(),
            "weighted_confusion_matrix": weighted_matrix.tolist(),
        }
    return result
...
def ABCD(config):
    ...
    diagnostics = {
        ...
        "class_assignment": class_assignment_diagnostics(
            labels_masked, masked_score_products, weights=weights_masked),
    }
    ...
    # 1D closure scan. Its range is independent of working-point selection.
    ...
    curve_percent = np.linspace(
        float(curve_percent_min), float(curve_percent_max),
    ...
    x_min_cfg = config.get("closure_curve_xmin")
    x_max_cfg = config.get("closure_curve_xmax")
    ...
    if x_min_cfg is not None or x_max_cfg is not None:
        left = float(x_min_cfg) if x_min_cfg is not None else (
            max(float(finite_eff.min()) * 0.8, 1e-6)
            if finite_eff.size else 1e-6)
        right = float(x_max_cfg) if x_max_cfg is not None else (
            min(float(finite_eff.max()) * 1.2, 1.0)
            if finite_eff.size else 1.0)
        if right <= left:
            raise ValueError(
                f"Invalid closure curve x-limits: xmin={left}, xmax={right}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--closure_curve_percent_min", type=float, default=None,
                        help="Lowest diagonal quantile used only for the closure curve.")
    parser.add_argument("--closure_curve_percent_max", type=float, default=None,
                        help="Highest diagonal quantile used only for the closure curve.")
    parser.add_argument("--closure_curve_percent_steps", type=int, default=None,
                        help="Number of diagonal points used only for the closure curve.")
    parser.add_argument("--closure_curve_xmin", type=float, default=None,
                        help="Optional lower x-axis limit for the closure curve.")
    parser.add_argument("--closure_curve_xmax", type=float, default=None,
                        help="Optional upper x-axis limit for the closure curve.")
